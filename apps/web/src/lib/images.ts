import { sha256Hex, type Attachment } from "@betapouch/core";
...
function assertAcceptable(file: File): void {
  if (file.size > MAX_INPUT_BYTES) {
    throw new ImageRejectedError(
      `That file is ${(file.size / 1024 / 1024).toFixed(1)} MB. The limit is ${MAX_INPUT_BYTES / 1024 / 1024} MB.`,
    );
  }
  if (file.size === 0) throw new ImageRejectedError("That file is empty.");
  // Trust the sniffed type over the extension; a file named .jpg is not
  // evidence of anything.
  if (!ACCEPTED_MIME.includes(file.type as (typeof ACCEPTED_MIME)[number])) {
    throw new ImageRejectedError(
      `${file.type || "That file type"} is not supported. Use a JPEG, PNG, WebP, HEIC or PDF.`,
    );
  }
}
...
async function decode(file: File): Promise<ImageBitmap> {
  try {
    ...
    throw new ImageRejectedError(
      "That image could not be decoded. HEIC files sometimes need to be exported as JPEG first.",
    );
  }
}
...
export async function prepareImage(
  ...
  if (file.type === "application/pdf" || !stripMetadata) {
    ...
    if (!context) throw new ImageRejectedError("This browser could not process the image.");
    ...
    if (!blob) throw new ImageRejectedError("This browser could not re-encode the image.");
  ...
  const attachment: Attachment = {
    id: newId("att"),
    mimeType: mimeType as Attachment["mimeType"],
  ...
  return {
    attachment,
    bytes,
    previewUrl: URL.createObjectURL(new Blob([bytes as BlobPart], { type: mimeType })),
  };
}
...
export function bytesToBase64Data(bytes: Uint8Array): string {
  let binary = "";
  const chunk = 0x8000;
...
export function filesFromDataTransfer(data: DataTransfer | null): File[] {
  ...
  return files.filter((f) => ACCEPTED_MIME.includes(f.type as (typeof ACCEPTED_MIME)[number]));
}
