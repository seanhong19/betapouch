 ...
 * consequences.
 */
function fileOf(bytes, type, name = "receipt.jpg") {
    return new File([new Uint8Array(bytes)], name, { type });
}
describe("prepareImage validation", () => {
    it("rejects an oversized file before decoding it", async () => {
        await expect(prepareImage(fileOf(26 * 1024 * 1024, "image/jpeg"))).rejects.toBeInstanceOf(ImageRejectedError);
    });
    ...
    it("rejects a type that is not an image or PDF, whatever the extension says", async () => {
        await expect(prepareImage(fileOf(100, "text/html", "totally-a-receipt.jpg"))).rejects.toBeInstanceOf(ImageRejectedError);
        await expect(prepareImage(fileOf(100, "application/x-msdownload", "receipt.png"))).rejects.toBeInstanceOf(ImageRejectedError);
    });
