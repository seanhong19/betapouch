...
/**
 * Vault lifecycle: create, unlock, auto-lock.
 *
 * The unlocked key lives in React state and nowhere else. It is never written
 * to storage, never put on `window`, and is dropped on lock — after which the
 * only way back in is the passphrase.
 */
...
interface VaultContextValue {
  ...
  settings: Settings;
  /** Number of consecutive failed unlock attempts, for the backoff notice. */
...
export function VaultProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void loadVaultHeader().then((header) => {
      if (!cancelled) setStatus(header ? "locked" : "uninitialised");
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    if (status !== "unlocked" || !settings.lockOnHide) return;
    const onVisibility = () => {
...
export function useUnlockedVault(): UnlockedVault {
