...
function getCrypto(provided) {
    const c = provided ?? globalThis.crypto;
    if (!c?.subtle) {
        throw new Error("WebCrypto is unavailable. BetaPouch requires a secure context (https:// or localhost).");
    }
    return c;
}
 ...
 */
function aadFor(store, recordId) {
    return utf8ToBytes(`betapouch:v1:${store}:${recordId}`);
}
async function deriveKek(crypto, passphrase, kdf) {
...
export function assessPassphrase(passphrase) {
    ...
    const variety = Number(/[a-z]/.test(value)) +
    ...
    const score = (problems.length ? Math.min(raw, 1) : raw);
...
export async function createVault(passphrase, options = {}) {
    ...
    if (!strength.ok)
        throw new Error(strength.problems.join(" "));
    ...
    const wrapped = new Uint8Array(await crypto.subtle.encrypt({ name: "AES-GCM", iv: toArrayBuffer(iv), additionalData: toArrayBuffer(aadFor("vault", "dek")) }, kek, toArrayBuffer(dekRaw)));
...
export async function unlockVault(header, passphrase, options = {}) {
    const crypto = getCrypto(options.crypto);
    if (header.version !== 1)
    ...
    try {
        ...
        }, kek, toArrayBuffer(base64ToBytes(header.wrappedDek.ciphertextB64))));
    }
    catch {
...
export async function changePassphrase(vault, currentPassphrase, newPassphrase, options = {}) {
    ...
    try {
        dekRaw = new Uint8Array(await crypto.subtle.decrypt({
            ...
            additionalData: toArrayBuffer(aadFor("vault", "dek")),
        }, oldKek, toArrayBuffer(base64ToBytes(vault.header.wrappedDek.ciphertextB64))));
    }
    catch {
        throw new WrongPassphraseError();
    }
    ...
    const iv = randomBytes(crypto, IV_BYTES);
    const wrapped = new Uint8Array(await crypto.subtle.encrypt({ name: "AES-GCM", iv: toArrayBuffer(iv), additionalData: toArrayBuffer(aadFor("vault", "dek")) }, newKek, toArrayBuffer(dekRaw)));
    wipe(dekRaw);
    return {
        ...vault.header,
        kdf,
...
export async function sealBytes(vault, store, recordId, plaintext) {
    const iv = randomBytes(vault.crypto, IV_BYTES);
    const ct = new Uint8Array(await vault.crypto.subtle.encrypt({ name: "AES-GCM", iv: toArrayBuffer(iv), additionalData: toArrayBuffer(aadFor(store, recordId)) }, vault.dek, toArrayBuffer(plaintext)));
    return { ivB64: bytesToBase64(iv), ciphertextB64: bytesToBase64(ct) };
}
export async function openBytes(vault, store, recordId, blob) {
    ...
    }, vault.dek, toArrayBuffer(base64ToBytes(blob.ciphertextB64)));
    return new Uint8Array(plain);
}
export async function sealJson(vault, store, recordId, value) {
    const bytes = utf8ToBytes(JSON.stringify(value));
    const sealed = await sealBytes(vault, store, recordId, bytes);
    wipe(bytes);
    return sealed;
}
export async function openJson(vault, store, recordId, blob) {
    const bytes = await openBytes(vault, store, recordId, blob);
    const text = bytesToUtf8(bytes);
    wipe(bytes);
    return JSON.parse(text);
}
export async function sha256Hex(bytes, provided) {
    const crypto = getCrypto(provided);
    const digest = await crypto.subtle.digest("SHA-256", toArrayBuffer(bytes));
    return bytesToHex(new Uint8Array(digest));
}
/** Passphrase-derived encryption for a standalone backup file. */
export async function encryptBackup(payload, passphrase, options = {}) {
    const strength = assessPassphrase(passphrase);
    if (!strength.ok)
        throw new Error(strength.problems.join(" "));
    const crypto = getCrypto(options.crypto);
    ...
    const ct = new Uint8Array(await crypto.subtle.encrypt({ name: "AES-GCM", iv: toArrayBuffer(iv), additionalData: toArrayBuffer(aadFor("backup", "v1")) }, key, toArrayBuffer(plaintext)));
    wipe(plaintext);
...
export async function decryptBackup(envelope, passphrase, options = {}) {
    ...
    try {
        ...
        }, key, toArrayBuffer(base64ToBytes(envelope.ciphertextB64)));
    }
    catch {
        throw new WrongPassphraseError();
    }
    return JSON.parse(bytesToUtf8(new Uint8Array(plain)));
}
export { concatBytes };
