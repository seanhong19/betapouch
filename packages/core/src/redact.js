...
const RULES = [
    ...
    },
    {
        ...
        replacement: (match) => (match.replace(/\D/g, "").length >= 9 ? "[phone]" : match),
    },
    {
        name: "longDigits",
...
export function redactSensitive(input) {
    ...
    for (const rule of RULES) {
        text = text.replace(rule.pattern, (match) => {
            const out = typeof rule.replacement === "function" ? rule.replacement(match) : rule.replacement;
            if (out !== match)
                removed[rule.name] = (removed[rule.name] ?? 0) + 1;
            return out;
        });
    }
    return { text, removed };
...
export function hasRedactions(result) {
