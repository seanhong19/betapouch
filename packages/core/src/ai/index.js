...
export function createProvider(config, credentials) {
    const runtime = {
        kind: config.kind,
    ...
    switch (config.kind) {
        ...
        default:
            throw new AiRequestError(`Unknown provider type "${config.kind}".`);
    }
}
/**
 ...
 */
export function prepareTextForProvider(config, text) {
    if (!config.redactBeforeSend)
        return { text, removed: {} };
    return redactSensitive(text);
}
/** Convert a validated model response into a draft the form can render. */
export function extractedToDraft(extracted, fallbackCurrency = "USD") {
    const currency = normaliseCurrency(extracted.currency) ?? fallbackCurrency;
    const toMinor = (value) => value === null ? null : parseAmountToMinor(value, currency);
...
function parseIsoLoose(value) {
    ...
    if (year < 1990 || date.getTime() > Date.now() + 36 * 3600 * 1000)
        return null;
