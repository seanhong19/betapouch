 ...
 */
export function createAgentBridgeProvider(config) {
    const baseUrl = config.baseUrl?.trim() || DEFAULT_BASE_URL;
    const url = assertSafeEndpoint(baseUrl);
    if (!isLoopback(url)) {
        throw new AiRequestError("The agent bridge must run on this device (localhost). Remote bridges are not allowed.");
    }
    const token = config.credentials.bridgeToken?.trim();
    if (!token)
        throw new AiRequestError("No bridge token configured. Start the bridge to get one.");
    const headers = { authorization: `Bearer ${token}` };
    const agent = config.model?.trim() || "claude";
    const send = async (system, prompt, signal) => {
        const response = (await postJson(joinUrl(baseUrl, "v1/run"), { agent, system, prompt }, headers, { signal, timeoutMs: config.timeoutMs ?? 180_000 }));
        if (response.error)
            throw new AiRequestError(`Agent bridge: ${response.error}`);
        if (!response.text?.trim())
            throw new AiRequestError("The agent returned an empty response.");
    ...
    return {
        ...
        async extract(request) {
            if (!request.text.trim()) {
                ...
                throw new AiRequestError("The agent bridge works from OCR text. Enable on-device OCR, or use a vision provider.");
            }
            const raw = await send(EXTRACTION_SYSTEM_PROMPT, buildExtractionUserPrompt(request.text), request.signal);
