...
export function createOpenAiCompatibleProvider(config) {
    ...
    if (!model)
        throw new AiRequestError("No model configured for this provider.");
    const apiKey = config.credentials.apiKey?.trim();
    // A local server (LM Studio, llama.cpp) legitimately has no key.
    const headers = apiKey ? { authorization: `Bearer ${apiKey}` } : {};
    const send = async (body, signal) => textOf(await postJson(joinUrl(baseUrl, "chat/completions"), body, headers, {
        signal,
    ...
    return {
        ...
        async extract(request) {
            ...
            if (content.length === 0)
                throw new AiRequestError("Nothing to extract from.");
            const raw = await send({
                ...
                messages: [
                    ...
                    { role: "user", content: request.image ? content : content[0].text },
                ],
            }, request.signal);
