 ...
 */
export function createOllamaProvider(config: AiRuntimeConfig): AiProvider {
  ...
  return {
    ...
    async chat(request: ChatRequest): Promise<string> {
      ...
      if (!content?.trim()) throw new AiRequestError("The local model returned an empty response.");
      return content;
    },
