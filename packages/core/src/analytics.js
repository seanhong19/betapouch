...
import { CATEGORY_LABELS } from "./models.js";
export function filterExpenses(expenses, filter = {}) {
    ...
    return expenses.filter((expense) => {
        ...
        if (to !== null && at > to)
            return false;
        if (categories && !categories.has(expense.category))
            return false;
        if (merchants && !merchants.has(expense.merchant.toLowerCase()))
            return false;
        if (currencies && !currencies.has(expense.currency))
            return false;
        if (tags && !expense.tags.some((t) => tags.has(t.toLowerCase())))
            return false;
        if (filter.unreviewedOnly && expense.reviewed)
            return false;
        if (filter.minAmountMinor != null && expense.amountMinor < filter.minAmountMinor)
            return false;
        if (filter.maxAmountMinor != null && expense.amountMinor > filter.maxAmountMinor)
            return false;
        if (search) {
            const haystack = [
                expense.merchant,
...
export function sortByOccurredAt(expenses, direction = "desc") {
    const sign = direction === "asc" ? 1 : -1;
    return [...expenses].sort((a, b) => sign * (Date.parse(a.occurredAt) - Date.parse(b.occurredAt)));
}
...
export function bucketLabel(key, granularity, locale = "en-US") {
 ...
 */
export function timeSeries(expenses, granularity, options = {}) {
...
function toGroupTotals(entries) {
...
export function totalsByCategory(expenses, currency) {
...
export function totalsByMerchant(expenses, options = {}) {
    const relevant = options.currency
        ? expenses.filter((e) => e.currency === options.currency)
        : expenses;
    const entries = new Map();
...
export function summarisePeriod(expenses, options) {
    ...
    const previousTotalMinor = previous.length
        ...
        : null;
    const largest = inPeriod.reduce((max, e) => (max === null || e.amountMinor > max.amountMinor ? e : max), null);
    return {
        ...
        largest,
        previousTotalMinor,
        changeRatio: previousTotalMinor === null || previousTotalMinor === 0
            ? null
 ...
 */
export function buildExpenseSummary(expenses, options = {
    currency: "USD",
}) {
    ...
    if (scoped.length === 0)
        return "The user has no expenses recorded in this currency yet.";
    ...
    const lines = [
        ...
        "",
        "By category:",
        ...totalsByCategory(scoped, currency).map((g) => `- ${g.label}: ${money(g.totalMinor)} across ${g.count} (${Math.round(g.share * 100)}%)`),
        "",
        `Top merchants:`,
        ...totalsByMerchant(scoped, { currency, limit: options.maxMerchants ?? 10 }).map((g) => `- ${g.label}: ${money(g.totalMinor)} across ${g.count}`),
        "",
        "Monthly totals:",
        ...
        ...sortByOccurredAt(scoped, "desc")
            ...
            .map((e) => `- ${e.occurredAt.slice(0, 10)} ${e.merchant || "Unknown"} ${minorToDecimalString(e.amountMinor, currency)} ${CATEGORY_LABELS[e.category]}`),
    ];
