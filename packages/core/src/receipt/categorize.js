 ...
 */
export function normaliseMerchantKey(merchant) {
    return merchant
        .toLowerCase()
        .replace(/#\s*\d+/g, " ")
        .replace(/\bno\.?\s*\d+\b/g, " ")
...
export function learnCategory(memory, merchant, category) {
    const key = normaliseMerchantKey(merchant);
    if (!key)
        return memory;
    return { ...memory, [key]: category };
}
export function recallCategory(memory, merchant) {
    const key = normaliseMerchantKey(merchant);
    return (key && memory[key]) || null;
}
/** Memory first, then keywords, then `other`. */
export function resolveCategory(memory, merchant, text) {
