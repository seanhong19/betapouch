...
/**
 * Heuristic receipt parsing over OCR text.
 *
 * This runs entirely on-device and is the *default* path — AI extraction is an
 * optional upgrade, never a requirement. A user with no API key and no network
 * still gets a filled-in draft they can correct.
 */
...
export interface ParsedReceipt {
  ...
  lineItems: { description: string; totalAmountMinor: number }[];
  category: Category | null;
  /** 0..1 — how much of the useful signal we actually found. */
  confidence: number;
}
...
const CODE_PATTERN =
  /\b(USD|EUR|GBP|JPY|CNY|HKD|SGD|AUD|NZD|CAD|CHF|SEK|NOK|DKK|PLN|CZK|INR|IDR|MYR|THB|VND|PHP|KRW|TWD|ZAR|BRL|MXN|AED|SAR|TRY|ILS)\b/;

const NOISE_LINE =
  /^(?:[-=*_.\s]{3,}|thank you|thanks|welcome|customer copy|merchant copy|receipt|invoice|tel|phone|fax|www\.|https?:)/i;
...
function extractAmounts(line: string, currency: string): { minor: number; raw: string }[] {
  const out: { minor: number; raw: string }[] = [];
  for (const match of line.matchAll(AMOUNT_PATTERN)) {
    ...
    const looksMonetary =
      /[.,]\d{1,2}\s*\)?$/.test(raw) || /[$£€¥₹₩₽₺₫₱]/.test(raw) || digits.length >= 4;
...
function clampDate(year: number, month: number, day: number): Date | null {
  ...
  const now = Date.now();
  if (d.getTime() > now + 36 * 3600 * 1000) return null;
 ...
 */
export function extractDate(text: string, dayFirst = false): Date | null {
  // ISO first: unambiguous.
  const iso = text.match(/\b(\d{4})-(\d{2})-(\d{2})\b/);
  if (iso) {
    const d = clampDate(Number(iso[1]), Number(iso[2]), Number(iso[3]));
    if (d) return withTime(d, text);
  }

  // 12 Mar 2025 / Mar 12, 2025
  const named = text.match(
    /\b(?:(\d{1,2})\s+([A-Za-z]{3,9})|([A-Za-z]{3,9})\s+(\d{1,2}))[,\s]+(\d{2,4})\b/,
  );
  if (named) {
...
function guessMerchant(lines: string[]): string | null {
  for (const line of lines.slice(0, 6)) {
    ...
    if (/\d{3,}/.test(trimmed)) continue; // address/phone/receipt number
...
function extractLineItems(lines: string[], currency: string, totalMinor: number | null) {
  const items: { description: string; totalAmountMinor: number }[] = [];
  const stopLabels = [...TOTAL_LABELS, ...SUBTOTAL_LABELS, ...TAX_LABELS, ...TIP_LABELS];
  for (const line of lines) {
    if (stopLabels.some((l) => l.test(line))) continue;
    if (NOISE_LINE.test(line)) continue;
    const amounts = extractAmounts(line, currency);
    const amount = amounts.at(-1);
    if (!amount || amount.minor <= 0) continue;
    if (totalMinor !== null && amount.minor > totalMinor) continue;
...
export function parseReceiptText(
  rawText: string,
  options: { dayFirst?: boolean; defaultCurrency?: string } = {},
): ParsedReceipt {
  ...
  let paymentHint: "cash" | "card" | null = null;
  if (CARD_LABELS.test(text)) paymentHint = "card";
  else if (CASH_LABELS.test(text)) paymentHint = "cash";
  ...
  let score = 0;
  let weight = 0;
  const add = (found: boolean, w: number) => {
    weight += w;
    if (found) score += w;
  };
