...
import { CATEGORY_LABELS, expenseSchema } from "./models.js";
/**
 * CSV export.
 *
 ...
 */
export function importExpensesJson(raw) {
    let parsed;
    try {
    ...
    if (!Array.isArray(list))
        throw new Error("No expense array found in that file.");
    if (list.length > 100_000)
        throw new Error("That file has too many records to import.");
    const expenses = [];
    const skipped = [];
    list.forEach((candidate, index) => {
        ...
        if (result.success)
            ...
            skipped.push({ index, reason: result.error.issues[0]?.message ?? "invalid record" });
    });
