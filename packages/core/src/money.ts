...
export function minorToDecimalString(minor: number, currency = "USD"): string {
  const exponent = currencyExponent(currency);
...
export function formatMoneyCompact(minor: number, currency = "USD", locale?: string): string {
  const exponent = currencyExponent(currency);
